...
# 4. Download the Helium Browser source code and FIX the broken AUR package script
RUN git clone https://aur.archlinux.org/helium-browser.git
WORKDIR /home/builduser/helium-browser
RUN sudo pacman -S --noconfirm pacman-contrib
RUN sed -i 's/^pkgver=.*/pkgver=0.11.2.1/' PKGBUILD
RUN sed -i 's/^pkgrel=.*/pkgrel=1/' PKGBUILD
RUN updpkgsums
